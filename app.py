...
p, div, span, label { color: var(--text); font-family: 'DM Sans', sans-serif; }
...
.stTextInput label { color: var(--muted) !important; font-size: 13px !important; }
...
.stSelectbox label { color: var(--muted) !important; font-size: 13px !important; }

[data-testid="stImage"] img { border-radius: 8px !important; display: block !important; }
...
<div style="position:relative;margin-bottom:4px;">
    <img src="{poster}" style="width:100%;border-radius:8px;display:block;"
         onmouseover="this.style.transform='scale(1.04) translateY(-3px)';this.style.boxShadow='0 12px 32px rgba(0,0,0,0.7)'"
...
def fetch_details(movie_title):
    try:
        ...
        data    = requests.get(search, timeout=5).json()
        ...
        with concurrent.futures.ThreadPoolExecutor() as ex:
            f_info    = ex.submit(requests.get,
                f"https://api.themoviedb.org/3/movie/{movie_id}?api_key={API_KEY}", timeout=5)
            f_videos  = ex.submit(requests.get,
                f"https://api.themoviedb.org/3/movie/{movie_id}/videos?api_key={API_KEY}", timeout=5)
            f_credits = ex.submit(requests.get,
                f"https://api.themoviedb.org/3/movie/{movie_id}/credits?api_key={API_KEY}", timeout=5)
            f_omdb    = ex.submit(requests.get,
                f"http://www.omdbapi.com/?t={cleaned}&apikey={OMDB_KEY}", timeout=5)
...
<div style="text-align:center;padding:3rem 0 2rem;">
    <div style="font-family:'Bebas Neue',sans-serif;
        ...
        CINE<span style="color:#e50914;">MATCH</span><span style="font-size:0.35em;vertical-align:super;color:#f5c518;letter-spacing:0.12em;">AI</span>
    </div>
    <p style="font-family:'DM Sans',sans-serif;font-size:14px;
              letter-spacing:0.22em;color:#8a8a99;text-transform:uppercase;margin:0;">
...
<div style="padding:20px;text-align:center;
    ...
    <p style="color:#8a8a99;font-size:14px;margin:0;">
        No movies saved yet — add some from recommendations below!
    </p>
</div>""", unsafe_allow_html=True)
...
selected_movie = st.selectbox("Select", movie_list, label_visibility="collapsed")
st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

if st.button("Get Recommendations", use_container_width=True):
    if selected_movie not in ["— Select a movie —", "No results found"]:
...
if st.session_state.selected_movie_details:
    ...
    st.markdown(
        "<hr style='border-color:rgba(255,255,255,0.07);margin:2.5rem 0 1.5rem;'>",
        unsafe_allow_html=True
    )
...
<h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(32px,5vw,56px);
    ...
    if details:
        ...
        with stat_cols[0]:
            detail_stat("IMDb Rating", f"⭐ {details['rating']}")
        with stat_cols[1]:
            detail_stat("Rotten Tomatoes", f"🍅 {details['rt']}")
        with stat_cols[2]:
            detail_stat("Runtime", f"{details['runtime']} min")
        with stat_cols[3]:
            detail_stat("Release", str(details['release_date'])[:4])
...
<div style="background:#111118;border:1px solid rgba(255,255,255,0.06);
    ...
    <div style="font-family:'Bebas Neue',sans-serif;font-size:1rem;
                letter-spacing:0.1em;color:#8a8a99;margin-bottom:8px;">OVERVIEW</div>
    <p style="font-family:'DM Sans',sans-serif;font-size:15px;
              line-height:1.7;color:#d0cfd8;margin:0;">{details['overview']}</p>
</div>""", unsafe_allow_html=True)
...
<p style="font-family:'DM Sans',sans-serif;font-size:16px;
          font-weight:500;color:#f0eff4;margin:12px 0 0;">{director['name']}</p>
""", unsafe_allow_html=True)
        ...
        if details.get("cast"):
            ...
            for i, actor in enumerate(details["cast"]):
                with cast_cols[i]:
                    ...
                    st.markdown(f"""
<p style="font-family:'DM Sans',sans-serif;font-size:12px;font-weight:500;
          color:#f0eff4;margin:4px 0 0;text-align:center;">{actor['name']}</p>
""", unsafe_allow_html=True)
